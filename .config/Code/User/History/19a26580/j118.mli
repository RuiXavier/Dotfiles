...
(*Erro de syntax no (old list1.view) @ (old list2.view)*)
(* r = append list1 list2
    ensures r.view = (old list1.view) @ (old list2.view)
    ensures r.length = (old list1.length) + (old list2.length) *)
